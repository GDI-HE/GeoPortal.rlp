...
def get_data_other(request: HttpRequest, catalogue_id):
    ...
    print_debug(EXEC_TIME_PRINT % ("search all resources for facet counts", time.time() - start_time))
    
    # Extract counts for all resources
    all_resource_counts = {}
    for resource_key in all_resource_keys:
        try:
            # Get nresults from the nested structure: resource[resource][resource][md][nresults]
            if (resource_key in search_results_all and 
                ...
                if isinstance(md_data, dict):
                    all_resource_counts[resource_key] = md_data.get("nresults", 0)
                else:
                    all_resource_counts[resource_key] = 0
            else:
                all_resource_counts[resource_key] = 0
        except (KeyError, TypeError, AttributeError) as e:
            print_debug(f"Error extracting count for {resource_key}: {e}")
            all_resource_counts[resource_key] = 0
    ...
    return GeoportalJsonResponse(html=view_content, params={}, resources=requested_resources).get_response()
...
def _get_page_primary(request, post_params, resource, page, lang_code, host):
    ...
    Args:
        request: The incoming request
        post_params: POST parameters dict
        resource: The resource type being paginated (e.g. 'dataset', 'wms')
        page: The requested page number
        lang_code: Language code
        host: Hostname
    Returns:
        JsonResponse with partial HTML
    """
    # Template mapping for primary resources
    primary_templates = {
    ...
    start_time = time.time()
    # Search only the single resource
    catalogue_id = PRIMARY_CATALOGUE
    searcher = Searcher(
        ",".join(keywords),
        [resource],
    ...
    # Calculate pages for this single resource
...
def _get_page_other(request, post_params, resource, page, lang_code, host, catalogue_id, source):
    ...
    Args:
        request: The incoming request
        post_params: POST parameters dict
        resource: The resource type being paginated
        page: The requested page number
        lang_code: Language code
        host: Hostname
        catalogue_id: The catalogue ID (DE or EU)
        source: 'de' or 'eu'
    Returns:
        JsonResponse with partial HTML
    """
    # Template mapping for other resources
    other_templates = {
    ...
    # The other templates expect the resource data as a direct variable
    # (set via {% with search_results.X.X as X %} in the parent template)
